...
{
  this.e = Big.biFromHex(encryptionExponent);
  this.d = Big.biFromHex(decryptionExponent);
  this.m = Big.biFromHex(modulus);
	...
	////////////////////////////////// TYF
  this.digitSize = 2 * Big.biHighIndex(this.m) + 2;
	this.chunkSize = this.digitSize - 11; // maximum, anything lower is fine
	////////////////////////////////// TYF

	this.radix = 16;
  this.barrett = new BarrettMu.BarrettMu(this.m);
}
...
// RSACryptoServiceProvider implementation.
{
    ...
    var text = key.radix == 16 ? Big.biToHex(crypt) : Big.biToString(crypt, key.radix);
		result += text + " ";
	}
