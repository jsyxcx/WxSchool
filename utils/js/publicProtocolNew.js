  ...
  wx.getNetworkType({
    success: function (res) {
      ...
      if (networkType=='none'){
        ...
        var tempUrl = storageKeyName.INTERFACEGU;
        url = tempUrl + url;
        console.log('url:', url);
        //拼接登录需要的签名
        ...
        signHmacSHA1.sign(signTemp, 'jsy309', function (sign) {
          //合并对象
          var tempData = assignObject(encryData, commonData);
...
var postDataEncry1 = function (encryData, commonData, flag) {
  ...
  for (var tempData in encryData) {
    ...
    // encrypt_rsa = RSA.KEYUTIL.getKey(storageKeyName.PUBLICKEY);
    // var encStr = encrypt_rsa.encrypt(encryData[tempData])
    // encStr = RSA.hex2b64(encStr);
    // console.log("加密结果：" + encStr)
    // //对value进行加密
    // var encryptStr = encStr;
    //修改值
